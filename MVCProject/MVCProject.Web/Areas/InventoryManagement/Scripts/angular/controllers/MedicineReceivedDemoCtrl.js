(function () {
    ...
    angular.module("DorfKetalMVCApp").controller('MedicineOpeningStockDemoCtrl', [
            '$scope', '$q', '$rootScope', '$ngBootbox', '$filter', 'CommonFunctions', 'CommonEnums', 'MedicineReceivedDemoService', 'CommonService', MedicineOpeningStockDemoCtrl
    ]);

    function MedicineOpeningStockDemoCtrl($scope, $q, $rootScope, $ngBootbox, $filter, CommonFunctions, CommonEnums, MedicineReceivedDemoService, CommonService) {
        ...
        $scope.medicineAutoCompleteUrl = MedicineReceivedDemoService.MedicineAutoCompleteUrl();

        //Auto Complete URL for Medicine Received From Details based on Received From Type.
        $scope.medicineReceivedFromAutoCompleteUrl = MedicineReceivedDemoService.MedicineReceivedFromAutoCompleteUrl();
        ...
        $scope.Init = function (GroupId) {
            ...
            $q.all([
               ...
               MedicineReceivedDemoService.GetMedicineReceivedDetails($scope.Filter)
        ...
        $scope.GetMedicineReceivedDetails = function () {
            MedicineReceivedDemoService.GetMedicineReceivedDetails($scope.Filter).then(function (response) {
            ...
            if (parseFloat(actionToEdit.Price) > 0 && parseInt(actionToEdit.Quantity) > 0) {
                ...
                if (frmMedicineDetailsEdit.$valid) {
                    if (actionToEdit.MedicineReceivedId > 0) {
                        MedicineReceivedDemoService.GetMinimumQtyForReceivedStock(actionToEdit.GroupId, actionToEdit.MedicineId, moment(actionToEdit.ExpiryDate).endOf('month').format('MMMM-YYYY')).then(function (response) {
                            $rootScope.isAjaxLoadingChild = true;
                            if (response && response.data) {
                                var data = response.data;
                                if (!angular.isUndefined(data)) {
        ...
        $scope.SaveMedicineReceivedDetails = function (medicineDetails, frmMedicineReceived) {
            ...
            MedicineReceivedDemoService.SaveMedicineReceivedDetails(medicineDetails).then(function (response) {
                if (response && response.data) {
                    var data = response.data;
                    showtoastr(data.Message, data.MessageType);
                    if (data.MessageType == messageTypes.Success) {// Success
